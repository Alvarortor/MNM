...
use DateTime;
#Modified original MUMi program so that it just needs to input the MUMs file and the 2 FastA sequences. 
#Order of input sequences does not matter
my $noFile = "does not exist";
my $blank = "is blank" ;
...
my $now = "$date $time";   # creates 'yyyy-mm-dd hh:mm:ss' string
...
#Clears previous enrties from error file so it can be overwritten
open my $ef, '>', "Error.log"

;
#How to clear the error file from here too 
#Need to create a mini error storage for these too
	#Open to make sure each file exists
	#Check to see that files aren't blank
	#Create error file if needed

open(M, $mums) or die "Could not open file: $!";
open(F1, $fh1) or die "Could not open file: $!";
open(F2, $fh2) or die "Could not open file: $!";


#Dummy line to remove header from FASTA files
my $Dummy1 = <M>;
my $Dummy2 = <F1>;
my $Dummy3 = <F2>;
...
#Count characters in reference file
while (<F1>) {
	chomp;
    $RefCount += length($_);
}
#Count characters in query file
while (<F2>) {
	chomp;
    $QCount += length($_);
}
...
#Aesthetics go here
#Remove extra line to get nice table
chomp $fh1;
chomp $fh2;
#Little description table
print "Length(bp)\t File Name\n";
print("$RefCount\t $fh1\n");
print("$QCount\t $fh2\n\n");
my $gen1_len = $RefCount;
my $gen2_len = $QCount;
#No its not broken, it just takes a bit of time
print "Calculating MUMi...\n";
;
...
#Everything below here is not my work, credit is to XXXX Team.
